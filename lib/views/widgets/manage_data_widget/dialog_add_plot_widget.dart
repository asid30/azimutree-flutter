...
class _DialogAddPlotWidgetState extends State<DialogAddPlotWidget> {
  ...
  void initState() {
    ...
    if (widget.clusters.isNotEmpty) {
      ...
      _selectedPlotCode = availableCodes.isNotEmpty ? availableCodes.first : null;
    }
  ...
  List<int> get _availablePlotCodesForSelectedCluster {
    ...
    final existingCodes = widget.plotNotifier.value
        .where((plot) => plot.idCluster == _selectedClusterId)
        .map((plot) => plot.kodePlot)
        .toSet();

    return List<int>.generate(4, (index) => index + 1)
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<int>(
              ...
              items: _availablePlotCodesForSelectedCluster
                  .map(
